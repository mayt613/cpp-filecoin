...
namespace CronActor = fc::vm::actor::builtin::cron;
...
TEST(CronActorTest, WrongSender) {
  ...
  EXPECT_OUTCOME_FALSE(err, CronActor::EpochTick::M::apply(actor, runtime, {}));
...
TEST(CronActorTest, Correct) {
  ...
  EXPECT_OUTCOME_TRUE_1(CronActor::EpochTick::M::apply(actor, runtime, {}));
}
