...
BlockProducerTest::CID BlockProducerTest::decodeCID(
    const std::vector<uint8_t> &data) const {
  EXPECT_OUTCOME_TRUE(id, CIDCodec::decode(data));
  return CID{std::move(id)};
}

BlockProducerTest::Address BlockProducerTest::getAddress(uint64_t id) const {
  return Address{Address::makeFromId(id)};
}
...
BlockProducerTest::EPostProof BlockProducerTest::getEPostProof() const {
  fc::common::Buffer proof_bytes{std::vector<uint8_t>(
      config::kPostProof.begin(), config::kPostProof.end())};
...
BlockProducerTest::getSampleMessages() const {
  ...
  UnsignedMessage message_A{getAddress(1),
                            getAddress(2),
                            123,
                            5,
                            100,
                            ...
                            MethodNumber{1},
                            MethodParams{}};
  UnsignedMessage message_B{getAddress(2),
                            getAddress(3),
                            456,
                            10,
                            120,
...
TEST_F(BlockProducerTest, Main) {
  ...
  EXPECT_CALL(*ipfs_datastore, get(decodeCID(config::kParentTipset)))
      .Times(1)
  ...
  InterpreterResult interpreter_result{decodeCID(config::kParentTipset),
                                       decodeCID(config::kParentTipset)};
  ...
  EXPECT_OUTCOME_TRUE(
      full_block,
      block_producer->generate(getAddress(config::kMinerAddressId),
                               decodeCID(config::kParentTipset),
                               getEPostProof(),
                               getTicket(),
                               std::make_shared<IndicesImpl>()));
  std::ignore = full_block;
}
