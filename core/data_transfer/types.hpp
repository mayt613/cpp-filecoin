...
namespace fc::data_transfer {
  ...
  //  /**
  //   * Voucher is used to validate a data transfer request against the
  //   underlying
  //   * storage or retrieval deal that precipitated it. The only requirement is
  //   a
  ...
  //    virtual outcome::result<std::vector<uint8_t>> toBytes() const = 0;
  //
  //    /** FromBytes reads a Voucher from raw bytes */
  //    virtual outcome::result<void> fromBytes(gsl::span<const uint8_t> byte) =
  //    0;
  //
   ...
   */
  using Subscriber =
      std::function<void(Event event, ChannelState channel_state)>;
