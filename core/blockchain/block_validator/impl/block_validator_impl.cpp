...
namespace fc::blockchain::block_validator {
  ...
  outcome::result<void> BlockValidatorImpl::consensus(
    ...
    OUTCOME_TRY(parent_tipset, get_parent_tipset(block));
  ...
  outcome::result<void> BlockValidatorImpl::block_sign(
      const BlockHeader &block) const {
  ...
  outcome::result<void> BlockValidatorImpl::election_post(
      const BlockHeader &block) const {
    return outcome::success();
  }

  outcome::result<void> BlockValidatorImpl::chain_ancestry(
      const BlockHeader &block) const {
    return outcome::success();
  }

  outcome::result<void> BlockValidatorImpl::message_sign(
      const BlockHeader &block) const {
    return outcome::success();
  }

  outcome::result<void> BlockValidatorImpl::state_tree(
      const BlockHeader &block) const {
    OUTCOME_TRY(parent_tipset, get_parent_tipset(block));
    OUTCOME_TRY(
        result,
  ...
  BlockValidatorImpl::get_parent_tipset(const BlockHeader &block) const {
